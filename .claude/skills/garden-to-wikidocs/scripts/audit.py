...
def main():
    ...
    expected_paths = [value["path"] for value in pages_map.values()]
    ...
    for gid, value in pages_map.items():
        ...
        source = garden / "content" / value["folder"] / f"{gid}.md"
        if not source.exists():
            errors.append(f"garden: 원본 없음: {source}")
            continue
        _, source_body = BUILD.split_frontmatter(source.read_text(encoding="utf-8"))
    ...
    print(f"[ok] TOC      : 챕터 {len(chapters)}개, 페이지 {len(children)}개")
    print(f"[ok] mapping  : {len(pages_map)}개, page_id {len(pages_map)-len(missing_ids)}개")
