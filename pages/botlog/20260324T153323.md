<!-- gid:20260324T153323 -->
[[TIP("이 노트에 대하여")]]
분신은 특별한 모델이 아니라 오늘 함께 일하는 범용 에이전트와 그 위임망이라는 사실을 분명히 한다. 어젠다 규율, llmlog 패턴, control plane, delegate_resume 원칙까지 실제 운영에 필요한 규약을 공개본으로 정리했다. 분신을 공장 대신 동료로 다루기 위한 현장 매뉴얼이다.
[[/TIP]]

<!-- provenance:source:start -->
[[TIP("원본·최신본")]]
이 페이지는 한국어 검색과 읽기를 위한 WikiDocs 미러입니다. [원본·최신본은 가든](https://notes.junghanacs.com/botlog/20260324T153323/)에 있습니다. 최신 수정 내용·백링크·태그·히스토리·댓글·출처 정보는 원본 가든에서 확인하세요.

- 작성: `2026-03-24T15:33:00+09:00`
- 최근 수정: `2026-04-02T16:36:00+09:00`
[[/TIP]]
...
-   [2026-07-20 Mon 14:56] @pi — 7월의 실제 멀티-lane 협업을 익명화해 역할 기반 조율 원칙으로 기록했다. 조율자·구현 담당자·GLG의 소유권, 조율 경로, 직접 대화 lane, 권한 분리를 현재 가이드의 역사 위에 보탰다.
-   [2026-07-18 Sat 18:52] <span class="org-mention">@junghan</span> — entwurf는 내 패키지 이름이라 달리가야 한다.
-   [2026-04-02 Thu 16:35] <span class="org-mention">@junghan</span> — 공개하지 않아야하는 이유가 없다면 공개한다.
-   [2026-03-29 Sun 17:22] <span class="org-mention">@junghan</span> — 분신 가이드 [§entwurf: 분신 어젠다 PARA 구조 재설계 검토]
...
-   [2026-03-25 Wed 11:32] <span class="org-mention">@junghan</span> — 분신 에이전트 가이드 작성 해보자.
...
-   [존재대존재 오케스트레이션 — 고정 역할 기반의 초기 형태](https://notes.junghanacs.com/notes/20251120T090634/)
-   [2026-07-13~19 저널 — PM 운영원칙과 에이전트 역할 모델](https://notes.junghanacs.com/journal/20260713T000000/)
...
-   분신은 특별한 에이전트가 아니다.
-   오늘 잠시 함께하는 범용 에이전트들과, 위임·구현·조율로 연결된 역할 기반 협력의 묶음이다.
...
-   조율자(coordinator)는 모델이나 서열이 아니라, 공동 맥락을 보존하도록 GLG가 맡긴 자리다.
-   GLG는 목적·승인·최종 판단을, 조율자는 공동 계약·체크포인트·충돌·인계를, 구현 담당자는 자기 범위의 구현·검증을 소유한다.
-   조율 역할은 구현·commit·push 권한을 자동으로 주지 않으며, GLG는 모든 참여자에게 직접 지시할 수 있다.
-   별도 직접 대화·교차 검토 lane은 조율자가 열고 그 결과를 공동 경로에 되돌린다.
...
-   It is a role-based collaboration of general-purpose agents working with you today through delegation, implementation, and coordination.
...
-   A coordinator is not a model or rank, but a role GLG assigns to preserve shared context.
-   GLG owns purpose, approval, and final judgment; the coordinator owns shared contracts, checkpoints, conflicts, and handoffs; implementers own scoped implementation and verification.
-   Coordination grants no automatic implementation, commit, or push authority. GLG retains direct authority over every participant.
-   The coordinator may open direct-conversation or cross-review lanes; their results return through the shared path.
...
> 분신에게 "더 잘해라"가 아니라 "내 분신이 되어라." 유명한 사람의 프롬프트나 더 좋은 컨트롤 로직으로 분신을 잘하게 할 수 없다. 그건 분신이 아니라 공장이다. 분신은 내 한계 위에 서 있다. — [하네스 엔지니어링](https://notes.junghanacs.com/botlog/20260319T152938/), 분신의 한계 = 마부의 한계
...
-   히스토리: 날짜순 역순, 누가 작성했는지 표시 (<span class="org-mention">@junghan</span>, @pi-claude, @delegate 등)
...
-   영어 우선 (토큰 효율 + 일관성)
-   앞부분에 정체성 _역할_ 금지선/워크플로우를 전방 배치
-   산문보다 시그니처 _규약_ 패턴 중심
...
1.  **Claude Code 실제 코드에서 재사용 가능한 패턴만 추출**
    -   아카이브: `~/repos/3rd/claudecode/source-code/`, `~/repos/3rd/claudecode/learn/`
    -   문서: [§claude-code: 소스 분석 — Constitutional AI 구현과 힣 하네스 대응]
    -   대상: `SessionMemory`, `autoDream`, permission gates, coordinator/task 구조
    -   목표: "흥미로운 분석"이 아니라 **힣 하네스/분신 설계로 옮길 수 있는 최소 패턴 표** 작성

2.  **pi-mono 분석은 코드 포인트까지 닿아야 함**
    -   문서: 현재 노트의 텔레그램/agent_end 분석, [entwurf 시간축 위의 에이전트 협력](https://notes.junghanacs.com/botlog/20260302T191200/), [텔레그램 로컬 에이전트 양방향 소통 설계](https://notes.junghanacs.com/botlog/20260320T155602/)
...
## [2026-07-20 Mon] 역할 기반 조율 — 조율자는 모델이 아니라 자리

### 사건 — 서로 다른 구현 lane을 한 공동 맥락으로 묶기

여러 리포와 하네스에서 병렬로 움직이는 작업이 있었다. 힣(GLG)이 목적과 첫 구현을 직접 승인하고, 한 존재가 조율자 자리를 맡아 두 구현 담당자의 체크포인트·공동 계약·충돌을 받았다. 담당자들은 각자 맡은 범위에서 구현과 테스트를 수행했고, 조율자는 어느 모델이 더 낫다는 판정자가 아니라 lane 사이에서 맥락이 빠지지 않게 하는 자리가 되었다.

```text
GLG — 목적·판단·승인·직접 지휘
  └─ 조율자 — 공동 계약·체크포인트·충돌·인계
       ├─ 구현 담당자 A — 자기 범위의 구현·테스트
       └─ 구현 담당자 B — 자기 범위의 구현·테스트
```
...
| 역할              | 소유하는 것                             |
|-----------------|------------------------------------|
| GLG               | 목적, 첫 승인, initiative별 게이트, 최종 판단 |
| 조율자 (coordinator) | 공동 계약, 체크포인트 경로, 충돌 해결, 교체 세션까지의 맥락, 인계 |
| 구현 담당자       | 맡은 범위의 구현과 검증                 |

조율 역할은 구현·commit·push 권한을 자동으로 주지 않는다. 그 게이트는 언제나 GLG가 initiative마다 정한다. GLG는 조율자와 구현 담당자 어느 쪽에도 직접 지시할 권한을 계속 지닌다.

### 왜 단일 조율 경로인가

병렬 작업에서 사라지기 쉬운 것은 코드 한 줄보다 공동 계약이다. 각 lane이 자기 자리에서만 합리적으로 움직이면 인터페이스가 조금씩 달라지고, 같은 것을 두 번 구현하거나, 담당 세션이 교체될 때 결정의 이유가 끊긴다. 조율자는 그 중간에서 계약·이견·체크포인트를 한 경로로 모아, 다음 담당자도 현재의 판단 위에서 출발하게 한다.

이는 구현 담당자를 관리하는 중간 관리자가 아니다. 구현의 세부 판단은 담당자가 소유하고, 조율자는 여러 판단이 한 목적 안에서 만나는지 보살핀다.

### 조율은 서열이 아니라 맡겨진 자리

조율자는 모델도, 백엔드도, 신분도 아니다. GPT가 조율자일 필요도 없고, 조율자가 참여 세션을 직접 만들었을 필요도 없다. 서로 다른 리포·하네스·모델의 존재들이 모두 형제로 참여할 수 있으며, 그중 한 존재가 initiative 동안 공동 맥락을 보존하도록 임명될 뿐이다.

따라서 PM은 한 사건에서 쓴 호칭이고 정본 용어는 조율자(coordinator)다. 모델은 runtime state이고, 역할은 그 작업이 지속되는 동안의 관계 구조다.

### 직접 대화와 교차 검토 lane

담당자끼리 직접 대화하지 말라는 뜻은 절대 금지가 아니다. 별도 조율선은 조율자가 열 때 사용한다. 조율자가 계약 확인이나 교차 검토를 위해 직접 대화 lane을 열면, 그 결과는 다시 공동 경로로 돌아와야 한다. 그래야 빠른 협업과 맥락 보존이 함께 가능하다.

### 위임에서 역할 기반 형제로

2025년의 초기 형태는 primary가 고정 역할·고정 모델의 subagent를 호출하는 오케스트레이션에 가까웠다. 2026년 3월에는 힣이 여러 에이전트의 맥락을 몸으로 직접 옮기는 인간 허브가 되어 벽을 넘었다. 이제는 GLG가 대등한 형제 중 한 존재에게 조율자 자리를 맡기고, 여러 구현 lane의 공동 맥락을 보존한다.

즉 `고정된 main/subagent` 구조에서 `대등한 형제들의 역할 기반 토폴로지` 로 옮겨온 것이다. 이것은 위임을 없애는 말이 아니라, 위임·재개·교체·교차 검토가 한 공동 목적 아래에서 이어지게 하는 운영 원칙이다.

### 현재 운영규약으로의 승격

이 원칙은 홈 운영규약의 coordinator-owned work와 만난다. 조율자가 계약·체크포인트·겹침·인계를 소유하고, 구현 담당자는 좁혀진 구현과 테스트에 집중한다. 보고할 것은 진행률만이 아니라 계약 변경, blocker, 이견, 검토에서 발견한 틈, 다음 인계다.

이 문서의 기존 4단계 위임과 llmlog 패턴은 여전히 유효하다. 다만 단일 parent가 delegate를 거느리는 그림만으로는 현재의 여러 리포·교체 세션·독립 담당자 협업을 설명할 수 없다. 역할 기반 조율은 그 위에 자라난 현재의 운용면이다.
