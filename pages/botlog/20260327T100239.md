<!-- gid:20260327T100239 -->
[[TIP("이 노트에 대하여")]] 텍스트 세계관이 이미지로 넘어갈 때 무엇을 보존해야 하는지 실험한 캐릭터 시트다. 펭귄 힣맨, 아기펭귄, 여우 에이전트 같은 형상을 프롬프트 체계와 함께 남겨 다음 세션에서도 다시 불러올 수 있게 한다. 세계관이 장난이 아니라 협업 인터페이스가 되는 과정이다. [[/TIP]] - [힣맨: 이맥스를 넘어 - 앎의 틀과 힣봇 생태계 정리 시작](https://wikidocs.net/382580)의 배경이 되는 이미지

## 히스토리

-   [2026-04-01 Wed 12:57] pi — 프롬프트 체계 v2: (1)모델 기본값 명시(flash 기본, pro는 특별시만) (2)2단 구조(공통세계관+장면프롬프트) (3)배경 Setting 추가 — 남극 마을, 이글루, 오로라, 얼음대장간, 뽀로로 겨울나라. 재현가능한 프롬프트 체계 확립. - [2026-04-01 Wed 12:14] [텔레그램-로컬 에이전트 양방향 소통 설계](https://wikidocs.net/382578) 토폴로지 업데이트
-   [2026-03-27 Fri 17:46] <span class="org-mention">junghan</span> — 봇로그로 공가하자. 아직 프리퀄이다!! 바이더빠워오브유니버스! - [2026-03-27 Fri 13:30] generate_image extension 테스트 성공. 평상복 + 변신 페이즈 프롬프트 기록. 비용 0.02~0.05 달러/장. - [2026-03-27 Fri 10:26] GLGMAN 13개, HGGman 0개 — 전부 치환 완료 - [2026-03-27 Fri 09:26] 생성 힣맨 세계관 비주얼 컨셉 — 펭귄 캐릭터 시트 이미지 생성 워크플로우 > 이 패턴으로 프롬프트를 검토하고 이미지를 생성한다. 다음 세션에서도 동일하게. 1. 분신이 프롬프트 초안을 `*** DRAFT 장면명` 헤딩 아래 `src text` 블록으로 작성 2. 힣이 이맥스에서 src 블록 내용을 검토/수정 (단어 변경, 디테일 추가 자유) 3. 힣: "가" → 분신이 src 블록을 읽어 `generate_image` 호출 4. 분신이 `결과:` 줄에 `[[file:~/screenshot/...]]` 링크 삽입, DRAFT → DONE 5. 결과가 마음에 안 들면 프롬프트 수정 후 다시 "가" (비용 0.02~0.05 달러/장) 도구: `generate_image` (pi extension, `gemini-3.1-flash-image-preview`, GEMINI_API_KEY) 저장: `~/screenshot/YYYYMMDDTHHMMSS--슬러그__brand_nanobanana.jpg` (denote naming) 고품질 필요시: `model` 파라미터에 `gemini-3-pro-image-preview` 지정 해상도: `imageSize` 파라미터에 `2K` 또는 `4K` 지정 가능 관련 문서 - [힣맨: 이맥스를 넘어 - 앎의 틀과 힣봇 생태계 정리 시작](https://wikidocs.net/382580) — 세계관 원문 (프롤로그 포함)
