...
1.  Omitting uninteresting files from Dired listing (see [BROKEN LINK: Omitting-Files-in-Dired]).
    
    Dired 목록에서 관심 없는 파일 생략하기(Dired 에서 파일 생략하기 참조).

2.  Running Dired command in non-Dired buffers (see [BROKEN LINK: Virtual-Dired]).
    
    Dired 이 아닌 버퍼에서 Dired 명령을 실행합니다(가상 Dired 참조).

3.  Finding a file mentioned in a buffer (see [BROKEN LINK: Find-File-At-Point]).
    
    버퍼에 언급된 파일 찾기(특정 지점에서 파일 찾기 참조).
4.  Commands using file marking (see [BROKEN LINK: Advanced-Mark-Commands]).
    
    파일 마킹을 사용하는 명령(고급 마크 명령 참조).

`dired-x.el` binds some functions to keys in Dired Mode (see [BROKEN LINK: Key-Index]). Optionally, it also binds C-x C-f and C-x 4 C-f to `dired-x-find-file` and `dired-x-find-file-other-window`, respectively (see [BROKEN LINK: Find-File-At-Point]).
...
When `dired-x.el` is loaded, some standard Dired functions from `dired.el` and `dired-aux.el` offer additional features. `dired-add-entry` obeys Dired Omit mode (see [BROKEN LINK: Omitting-Files-in-Dired]), if it is active. `dired-find-buffer-nocreate` and `dired-initial-position` respect the value of `dired-find-subdir` (see [BROKEN LINK: Miscellaneous-Commands]).
...
If you choose to have `dired-x.el` bind `dired-x-find-file` over `find-file` (see [BROKEN LINK: Find-File-At-Point]), then you will need to set `dired-x-hands-off-my-keys`. To do this, either set it _before_ `dired-x.el` is loaded, or use M-x customize-variable, or call `dired-x-bind-find-file` after changing the value.
...
In order to make Dired Omit work you need to load `dired-x` after loading `dired` (see [BROKEN LINK: Installation]) and then evaluate `(dired-omit-mode 1)` in some way (see [BROKEN LINK: Omitting-Variables]).

Dired Omit 을 작동시키려면 `dired` (설치 참조)를 로드한 후 `dired-x` ( )를 로드한 다음 `(dired-omit-mode 1)` (변수 생략 참조)를 어떤 방식으로든 평가해야 합니다.

-   [BROKEN LINK: Omitting-Variables]
...
Next: [BROKEN LINK: Omitting-Examples], Up: [BROKEN LINK: Omitting-Files-in-Dired] [[BROKEN LINK: SEC_Contents]][[BROKEN LINK: Concept-Index]]
...
-   Variable: **dired-omit-mode** [BROKEN LINK: index-dired_002domit_002dmode-1]
    ...
    (dired-omit-mode 1)
    ```
    inside your `dired-mode-hook` to have omitting initially turned on in _every_ Dired buffer (see [BROKEN LINK: Installation]). You can then use C-x M-o to unomit in that buffer.
...
-   Variable: **dired-omit-files** [BROKEN LINK: index-dired_002domit_002dfiles]
    ...
    The default value omits the special directories `.` and `..` and autosave files (plus other files ending in `.`) (see [BROKEN LINK: Omitting-Examples]).
    
    기본값은 `.` 및 `..` 특수 디렉터리와 자동 저장 파일( `.` 으로 끝나는 기타 파일)을 생략합니다(다양한 파일 유형 생략 예 참조).

-   Variable: **dired-omit-extensions** [BROKEN LINK: index-dired_002domit_002dextensions]
...
-   Variable: **dired-omit-case-fold** [BROKEN LINK: index-dired_002domit_002dcase_002dfold]
...
-   Variable: **dired-omit-localp** [BROKEN LINK: index-dired_002domit_002dlocalp]
...
-   Variable: **dired-omit-marker-char** [BROKEN LINK: index-dired_002domit_002dmarker_002dchar]
...
Next: [BROKEN LINK: Omitting-Technical], Previous: [BROKEN LINK: Omitting-Variables], Up: [BROKEN LINK: Omitting-Files-in-Dired] [[BROKEN LINK: SEC_Contents]][[BROKEN LINK: Concept-Index]]
...
-   If you wish to avoid seeing RCS files and the `RCS` directory, then use
    ...
    |,v$"))
    ```
    after loading `dired-x` (see [BROKEN LINK: Installation]). This assumes `dired-omit-localp` has its default value of `no-dir` to make the `^`-anchored matches work. As a slower alternative, with `dired-omit-localp` set to `nil`, you can use `/` instead of `^` in the regexp.
...
-   If you use `tib`, the bibliography program for use with TeX and LaTeX, and you want to omit the `INDEX` and the `*-t.tex` files, then use
    ...
    .tex$"))
    ```
    after loading `dired-x` (see [BROKEN LINK: Installation]).
...
-   If you do not wish to see ‘=dot=' files (files starting with a `.`), then use
    ...
    ..+$"))
    ```
    after loading `dired-x` (see [BROKEN LINK: Installation]). (Of course, a better way to achieve this particular goal is simply to omit ‘=-a=' from `dired-listing-switches`.)
...
Previous: [BROKEN LINK: Omitting-Examples], Up: [BROKEN LINK: Omitting-Files-in-Dired] [[BROKEN LINK: SEC_Contents]][[BROKEN LINK: Concept-Index]]
...
`dired-mark-extension` [BROKEN LINK: index-dired_002dmark_002dextension] :: Mark all files with a certain extension for use in later commands. A ‘=.=' is automatically prepended to the string entered when not present. If invoked with prefix argument C-u, this command unmarks files instead. If called with the C-u C-u prefix, asks for a character to use as the marker, and marks files with it.
...
`dired-mark-suffix` [BROKEN LINK: index-dired_002dmark_002dsuffix]
...
`dired-flag-extension` [BROKEN LINK: index-dired_002dflag_002dextension] :: Flag all files with a certain extension for deletion. A ‘=.=' is _not_ automatically prepended to the string entered.
    
    특정 확장자를 가진 모든 파일을 삭제하도록 플래그를 지정합니다. 입력한 문자열에 ' `.` '가 자동으로 앞에 붙지 않습니다.

[BROKEN LINK: Advanced-Cleaning-Functions]

[BROKEN LINK: Advanced-Cleaning-Variables]

[BROKEN LINK: Special-Marking-Function]

### 5.1 Advanced Cleaning Functions

5.1 고급 청소 기능

`dired-clean-patch` [BROKEN LINK: index-dired_002dclean_002dpatch]
...
`dired-clean-tex` [BROKEN LINK: index-dired_002dclean_002dtex]
: Flag dispensable files created by TeX, LaTeX, and ‘=texinfo=' for deletion. See the following variables (see [BROKEN LINK: Advanced-Cleaning-Variables]):
...
`dired-very-clean-tex` [BROKEN LINK: index-dired_002dvery_002dclean_002dtex] :: Flag dispensable files created by TeX, LaTeX, ‘=texinfo=', and `*.dvi` files for deletion.
...
Next: [BROKEN LINK: Special-Marking-Function], Previous: [BROKEN LINK: Advanced-Cleaning-Functions], Up: [BROKEN LINK: Advanced-Mark-Commands] [[BROKEN LINK: SEC_Contents]][[BROKEN LINK: Concept-Index]]

### 5.2 Advanced Cleaning Variables

5.2 고급 청소 변수

Variables used by the above cleaning commands (and in the default value for variable `dired-omit-extensions`, see [BROKEN LINK: Omitting-Variables]):

위의 정리 명령에 사용되는 변수(및 변수의 기본값 `dired-omit-extensions` , 변수 생략 참조):

-   Variable: **dired-patch-unclean-extensions** [BROKEN LINK: index-dired_002dpatch_002dunclean_002dextensions]
...
-   Variable: **dired-tex-unclean-extensions** [BROKEN LINK: index-dired_002dtex_002dunclean_002dextensions]
...
-   Variable: **dired-texinfo-unclean-extensions** [BROKEN LINK: index-dired_002dtexinfo_002dunclean_002dextensions]
...
-   Variable: **dired-latex-unclean-extensions** [BROKEN LINK: index-dired_002dlatex_002dunclean_002dextensions]
...
-   Variable: **dired-bibtex-unclean-extensions** [BROKEN LINK: index-dired_002dbibtex_002dunclean_002dextensions]
...
Previous: [BROKEN LINK: Advanced-Cleaning-Variables], Up: [BROKEN LINK: Advanced-Mark-Commands] [[BROKEN LINK: SEC_Contents]][[BROKEN LINK: Concept-Index]]
...
M-( [BROKEN LINK: index-M_002d_0028]
...
Next: [BROKEN LINK: Find-File-At-Point], Previous: [BROKEN LINK: Advanced-Mark-Commands], Up: [BROKEN LINK: Top]   [[BROKEN LINK: SEC_Contents]][[BROKEN LINK: Concept-Index]]
...
In order to use this feature, you will need to set `dired-x-hands-off-my-keys` to `nil` before loading `dired-x` (see [BROKEN LINK: Optional-Installation-File-At-Point]).

이 기능을 사용하려면 `dired-x` 을 로드하기 전에 `dired-x-hands-off-my-keys` 을 `nil` 로 설정해야 합니다(설치 파일 선택 사항 참조).

`dired-x-find-file` [BROKEN LINK: index-dired_002dx_002dfind_002dfile]
...
`dired-x-find-file-other-window` [BROKEN LINK: index-dired_002dx_002dfind_002dfile_002dother_002dwindow] ::
...
`dired-x-hands-off-my-keys` [BROKEN LINK: index-dired_002dx_002dhands_002doff_002dmy_002dkeys] :: If set to `t`, then it means that `dired-x` should _not_ bind `dired-x-find-file` over `find-file` on keyboard. Similarly, it should not bind `dired-x-find-file-other-window` over `find-file-other-window`. If you change this variable after `dired-x.el` is loaded then do M-x dired-x-bind-find-file. The default value of this variable is `t`; by default, the binding is not done. See [BROKEN LINK: Optional-Installation-File-At-Point].
...
`dired-x-bind-find-file` [BROKEN LINK: index-dired_002dx_002dbind_002dfind_002dfile] :: A function, which can be called interactively or in your `~/.emacs` file, that uses the value of `dired-x-hands-off-my-keys` to determine if `dired-x-find-file` should be bound over `find-file` and `dired-x-find-file-other-window` bound over `find-file-other-window`. See [BROKEN LINK: Optional-Installation-File-At-Point].
...
-   `dired-find-subdir` [BROKEN LINK: index-dired_002dfind_002dsubdir] :: Default: `nil` 기본값입니다: `nil`
