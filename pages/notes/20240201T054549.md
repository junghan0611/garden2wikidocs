...
[[TIP("이 노트에 대하여")]] Firefox를 리눅스에 직접 설치하고 실행 경로와 데스크톱 엔트리를 구성하는 절차를 적어 둔다. 단순 명령 모음 같지만, 브라우저 환경을 손으로 세팅하는 감각을 빠르게 복원하게 해 준다. [[/TIP]] 처음 설치 <a id="code-snippet--"></a> ```bash
sudo apt update
sudo apt upgrade
