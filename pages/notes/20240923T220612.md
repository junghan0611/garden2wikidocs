<!-- gid:20240923T220612 -->
[[TIP("이 노트에 대하여")]]
일본어 위주로 발전한 textlint 생태계를 보며 한국어용 플러그인을 직접 만들 수 있을지 질문한다. 기술적 준비와 구현 방향이 대화형 메모 속에서 점차 구체화되는 작업 노트다.
[[/TIP]]

## BIBLIOGRAPHY

## [2024-09-23 Mon 22:06]

대략 파악 된다. 아. 이렇게 간단하게 할 수 있다니. 작정하고 덤벼들면 발레로 다 만들 수 있겠다.

[SYS]: You are a helpful assistant. Please write in Korean language. @user 혹시 textlint 프로젝트에 대해서 알고 있니?

@assistant 네, textlint 프로젝트에 대해서 알고 있습니다. textlint는 JavaScript로 작성된 오픈 소스 텍스트 린터입니다. 주로 문법, 스타일, 맞춤법 등의 오류를 찾아내고 수정하는 데 사용됩니다. 다양한 플러그인을 통해 확장할 수 있으며, 사용자 정의 규칙을 설정할 수도 있습니다. 특정 프로젝트나 문서의 품질을 유지하는 데 유용하게 사용될 수 있습니다. 특정한 질문이 있으시면 말씀해 주세요!

@user 그래. 주로 일본어 플러그인만 있는데, 유사한 목적으로 한글 플러그인을 만들고 싶어. 어떻게 하면 될까?

@assistant 한글 플러그인을 만들기 위해서는 다음 단계를 따를 수 있습니다:
...
@user 혹시 vale라는 린터도 알고 있니? 주로 영문을 지원하는 것 같아. 특히 yaml 파일로 규칙을 지정하면 여러가지 기존 린터들을 호출하는 구조인 것 같아. yale 용으로 위의 규칙을 지원하게 할 수 있을까?

@assistant 네, Vale에 대해서도 알고 있습니다. Vale는 다양한 스타일 가이드를 지원하는 플러그러블(pluggable) 문서 스타일 린터입니다. 사용자들은 YAML 파일을 사용하여 규칙을 정의하고 이를 통해 문서의 일관성을 유지할 수 있습니다.
...
2.  **YAML 파일로 규칙 정의**:
    ...
    -   설정 파일에 다음 내용을 추가합니다: \`\`\`ini StylesPath = styles [\*.md] BasedOnStyles = Korean \`\`\` 3. **규칙 파일 만들기**: - \`.vale/styles/Korean\` 디렉토리를 생성합니다. - 여기에 \`no-doubled-conjunctive.yml\` 파일을 생성하고 다음과 같이 규칙을 정의합니다: \`\`\`yaml extends: existence message: 중복된 접속사가 발견되었습니다: '1 달러' level: error scope: text match: \b(그리고 그리고)\b \`\`\` 4. **규칙 활성화**: - 설정 파일(\`.vale/vale.ini\`)에서 정의한 규칙을 활성화합니다: \`\`\`ini StylesPath = styles [\*.md] BasedOnStyles = Korean \`\`\` 5. **테스트 및 실행**: - 테스트할 Markdown 파일이나 텍스트 파일을 작성하고, 예를 들어 \`test.md\`에 다음과 같은 내용을 포함시킵니다: \`\`\` 이것은 올바른 문장입니다. 그리고 그리고 오류가 발생했습니다. \`\`\` - Vale를 실행하여 파일을 검사합니다: \`\`\`sh vale test.md \`\`\` 이 설정을 통해 Vale가 지정된 규칙에 따라 문서를 검사하고, "그리고 그리고"와 같은 중복 접속사를 발견하면 오류 메시지를 출력하게 됩니다. Vale는 매우 유연한 도구로, 다양한 문법 규칙을 YAML 파일을 통해 손쉽게 정의하고 사용할 수 있습니다. 추가적인 고급 기능과 설정 방법은 Vale의 공식 문서를 참고하면 도움이 됩니다. user Related-Notes - [린터: 발레 - 텍스트린트 비교](https://notes.junghanacs.com/notes/20231108T061821/)
