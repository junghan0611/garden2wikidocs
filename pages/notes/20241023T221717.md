...
[[TIP("이 노트에 대하여")]] popup frame을 이용해 org-capture나 AI 인터페이스를 시스템 전역에서 호출하는 방법을 정리한다. 워크스페이스를 유지한 채 필요한 기능만 꺼내 쓰려는 이맥스 클라이언트 감각이 인상적이다. [[/TIP]] [옴니](https://notes.junghanacs.com/meta/20241013T213110/) 인터페이스의 일환이다.

## 이맥스 클라이언트 인터페이스

[2024-10-23 Wed 12:01] 완벽한 방법이다. 워크스페이스 가져가니까. ```elisp
...
``` 레골리스 i3wm 설정 [2024-10-23 Wed 22:18] - /usr/share/regolith/common/config.d/15_base_launchers <!--listend--> ```text
...
``` - ~/mydotfiles/local/.local/bin/myai.sh <!--listend--> ```shell
#!/bin/bash
...
``` Related-Notes - [이맥스 팝업 레이아웃 창관리](https://notes.junghanacs.com/notes/20231125T083342/)
