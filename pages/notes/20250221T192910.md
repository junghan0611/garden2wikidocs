...
-   [2025-02-21 Fri 19:29] 이 걸로 끝낸다. [emacs-jupyter/ob-jupyter 조직모드 바벨 파이썬 주피터 매뉴얼](https://wikidocs.net/381236)
...
Citations: [1] <https://blog.naver.com/jodi999/222088290630?viewType=pc> [2] <https://docs.python.org/ko/3.12/reference/executionmodel.html> [3] <https://codewith.mu/en/tutorials/1.2/repl> user assistant 조직모드에서 `ob-python` 을 사용하여 코드 블록을 실행하고 결과를 REPL 버퍼로 보내기 위해서는 다음 단계를 따르세요: 1. **org-mode 설정**: `ob-python` 을 사용하기 위해 org-mode가 설치되어 있어야 합니다. org-mode가 설치되어 있지 않다면, Emacs 패키지 매니저를 통해 설치하세요. 2. **코드 블록 설정**: org 파일에서 코드 블록을 정의합니다. 예를 들어:     ```org
    ...
    print("Hello, World!")
    ``` \+end_src 4. **코드 블록 실행 및 REPL 연동**: - `C-c C-c` 로 코드 블록을 실행하면 결과는 org 파일에 삽입됩니다. - `M-x jupyter-repl-associate-buffer` 명령어로 현재 버퍼를 REPL과 연동할 수 있습니다[3][4]. 5. **코드 전송**: `jupyter-eval-line-or-region` 명령어를 사용하여 특정 코드 줄이나 영역을 REPL로 전송할 수 있습니다. 이를 위해 `jupyter-org-with-src-block-client` 함수를 사용하여 클라이언트를 설정할 수 있습니다[3]. 이 방법은 Jupyter 커널을 사용하여 코드 블록을 실행하고 결과를 REPL 버퍼로 전송하는 데 유용합니다. Citations: [1] <https://sqrtminusone.xyz/posts/2021-05-01-org-python/> [2] <https://github.com/emacs-jupyter/jupyter> [3] <https://github.com/nnicandro/emacs-jupyter/issues/266> [4] <https://discourse.julialang.org/t/jupyter-integration-with-emacs/21496> [5] <https://martibosch.github.io/jupyter-emacs-universe/> [2025-02-21 W07](https://wikidocs.net/380399) jupyter 패키지 활용
...
``` 따라한 스크린샷 ~/sync/code/junghan0611/data-science-from-scratch/myscratch/script.py ![](../../assets/20250222T191957--jupyter-repl-interaction.png) 근데 주피터 레플에 컬러가 없는가! [2025-02-22 Sat 19:23] 중요한 것은 아니니 [주피터 파이썬 - 리눅스 qtconsole::jupyter/qtconsole](https://wikidocs.net/381548.md#h-134d3bb6-730f-4a0a-880c-2a4050d08991/)
