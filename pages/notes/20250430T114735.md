...
$$ \alpha = \beta $$
``` 연습 문제 1.1.2 주피터 노트북을 하나 만들고 인라인 수식과 디스플레이 수식으로 알파부터 오메가까지 써본다. 고대 그리스 개념어 — 에우다이모니아에서 테크네까지 [2026-07-08 Wed 16:50] 이 절은 옛 [고대 그리스어 용어 - 프로네시스](https://wikidocs.net/381692) 방에서 합친 자료다. 그리스 문자 노트는 autholog가 아니라 참고 허브이므로, 문자·기호·개념어를 한곳에 둔다. 프로네시스, 에우다이모니아, 테크네 같은 단어는 힣의 가든에서 반복적으로 쓰이는 중요한 개념어다.
